...
export default function ImportSiswaPage() {
  ...
  async function handleUpload() {
    ...
    const res = await fetch('/api/v1/siswas/import/preview', {
      method: 'POST',
      body: formData,
    });
  ...
  async function handleKonfirmasi() {
    const siswaValid = hasil.filter((h) => h.status === 'valid').map((h) => h.data);
    if (siswaValid.length === 0) return;

    setMengimport(true);
    setError(null);

    const res = await fetch('/api/v1/siswas/import/commit', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ siswaList: siswaValid }),
    });
    const data = await res.json();
  ...
  return (
    <AppShell title="Import siswa dari Excel">
      <div
        style={{
          background: '#ffffff',
      ...
      {hasilImport && (
        ...
        >
          Import selesai: {hasilImport.berhasil} siswa berhasil disimpan
      ...
      {ringkasan && (
        <>
          ...
          {ringkasan.valid > 0 && (
            ...
            >
              {mengimport ? 'Menyimpan...' : `Konfirmasi & simpan ${ringkasan.valid} siswa`}
            </button>
          )}
        </>
      )}
